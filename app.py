...
globals_data = initialize_globals(app, logger, OPENVOICE_PATH, DEVICE_V1, DEVICE_V2, AUDIO_FILES_PATH, WATERMARK, USE_VAD)

# Unpack the returned globals
MODEL_LANGUAGES_CODES_V1, MODEL_LANGUAGES_NAMES_V1, reference_speakers, targets_v2, targets_v1, models, speaker_ids, ckpt_base, base_speaker_tts, tone_color_converter_v1, tone_color_converter_v2, STYLES_V1 = globals_data

Voice.set_vars(logger, MODEL_LANGUAGES_NAMES_V1, speaker_ids, OPENVOICE_PATH, models, ckpt_base, base_speaker_tts, WATERMARK)
ApiRequest.set_vars(models, MODEL_LANGUAGES_CODES_V1, MODEL_LANGUAGES_V2, SPEAKERS, MODEL_LANGUAGES_NAMES_V1, STYLES_V1, SUPPORTED_STYLES_V1, speaker_ids, reference_speakers)
...
async def generate_audio_openai(version):
    ...
    args = dict(await request.get_json())
    validation_result = await ApiRequest.validate_generate_audio_request(args, version, True)
...
async def generate_audio(version, args=None, source_file=None):
    ...
    try:
        ...
        if response_format == 'url':
            ...
            payload_response = ApiResponse.payload(True, 200, "Successfully converted text to audio", {'url': output_url})
            ...
            payload_response = ApiResponse.payload(True, 200, "Successfully generated base64 audio", {'audio_data': audio_base64})
            return await ApiResponse.output(payload_response, 200)
            ...
            payload_response = ApiResponse.payload(False, 400, f"Invalid response_format parameter: {raw_response_format}")
            return await ApiResponse.output(payload_response, 400)
